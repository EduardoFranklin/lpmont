      ...
      <div className="section-container relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-3xl mx-auto">
          <Accordion type="single" collapsible className="space-y-3">
            {faqs.map((faq, i) => (
              <AccordionItem key={i} value={`faq-${i}`} className="gradient-card rounded-2xl overflow-hidden border-none">
                <div className="gradient-card-inner rounded-2xl px-6">
                  <AccordionTrigger className="text-left text-[15px] sm:text-base font-medium text-foreground/80 hover:text-foreground py-5 hover:no-underline">{faq.q}</AccordionTrigger>
                  <AccordionContent className="text-foreground/30 text-[14px] leading-relaxed pb-5">{faq.a}</AccordionContent>
                </div>
              </AccordionItem>
            ))}
          </Accordion>
